...
data class FilmInfoModel(
    ...
    fun toModel() = FilmInfoUIModel(
        title = nameRu ?: "",
        description = description ?: "",
        genres = genres.mapNotNull { it.genre }.toString().replace("[", "").replace("]", ""),
        countries = countries.mapNotNull { it.country }.toString().replace("[", "").replace("]", ""),
        posterUrl = posterUrl ?: ""
    )
}